    ...
    success: function(res) {
        ...
        for(let i=res[0].length-1;i>=0;i--){
            ...
            $(".register").append(
                ...
                '<p class="alignleft">Tips: </p>'+
                '<p class="alignright">'+ res[0][i][11] +'$</p><br><br>'+
                '<div style="clear: both;"></div>'+
                '</div><br>'
            )
            if(res[0][i][13]=="Delivered")
                totalEarns = parseFloat(totalEarns) + parseFloat(res[0][i][11])
        }
        $('#total-earns').append(
            '<p class="alignleft">Total Earns: </p>'+
            '<p class="alignright">'+ totalEarns +'$</p><br><br>'
        )
...
$(document).on('click','.orderid', function(event) {
    ...
    if(dict[id]['status']!="Delivered"){window.location="./"}
    else{
    $.ajax({
        type:'POST',
        contentType:'application/json',
